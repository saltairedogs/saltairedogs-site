 ...
 * - Subtle progress bar
 * - No pricing/services claims; neutral CTA
 */
...
export default function PerfectRecallClient({
  showHero = true,
  stickyToc = true,
  showMetaBar = true, // NEW: allow parent to disable the inner top meta bar
}: {
  showHero?: boolean;
  stickyToc?: boolean;
  showMetaBar?: boolean;
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 pb-24 sm:px-6 lg:grid-cols-12 lg:px-8">
        ...
        <aside className="order-last space-y-6 lg:order-first lg:col-span-3">
          ...
          <div className="rounded-2xl border bg-white p-4 shadow-sm" style={{ borderColor: BRAND.hair }}>
            ...
            <QuizRow label="Looks when name is said" value={quiz.nameResponse} onChange={(v) => setQuiz((s) => ({ ...s, nameResponse: v }))} />
            <QuizRow label="Loose lead at home" value={quiz.leadSlack} onChange={(v) => setQuiz((s) => ({ ...s, leadSlack: v }))} />
            <QuizRow label="Treat value (interest)" value={quiz.treatValue} onChange={(v) => setQuiz((s) => ({ ...s, treatValue: v }))} />
            <QuizRow
              ...
              invert
            />
            <QuizRow label="Calm start (2–3 breaths)" value={quiz.startLine} onChange={(v) => setQuiz((s) => ({ ...s, startLine: v }))} />
          ...
          <div className="rounded-2xl border bg-white p-4 shadow-sm" style={{ borderColor: BRAND.hair }}>
            ...
            <CalcRow label="Treat value" description="1=meh, 5=wow" value={reinforcer.value} onChange={(v) => setReinforcer((s) => ({ ...s, value: v }))} min={1} max={5} />
            <CalcRow label="Rate per minute" description="treats/min" value={reinforcer.ratePerMinute} onChange={(v) => setReinforcer((s) => ({ ...s, ratePerMinute: v }))} min={2} max={12} />
            <CalcRow label="Session minutes" value={reinforcer.sessionMinutes} onChange={(v) => setReinforcer((s) => ({ ...s, sessionMinutes: v }))} min={1} max={5} />

            <div className="mt-3 flex items-center justify-between text-sm">
              <span style={{ color: BRAND.slate }}>Bring at least</span>
              <span className="rounded-md px-2 py-0.5 font-semibold" style={{ background: 'rgba(200,155,60,0.15)', color: BRAND.ink }}>
              ...
              {[440, 520, 560, 590, 620].map((hz) => (
                <button
                  ...
                  className={cx(
                    ...
                    whistleTone === hz ? 'border-emerald-300 bg-emerald-50 text-emerald-800' : 'border-[#E6E3DA] bg-white text-[#131415]/80 hover:bg-[#EFEEE9]'
                  )}
        ...
        <article ref={articleRef} className="lg:col-span-9">
          {showMetaBar ? <TopMetaBar /> : null}

          {/* Overview */}
          <Section
  id="intro"
  title="Perfect Recall Training Guide — reliable, kind, and practical"
  srOnlyTitle
>
    ...
    <Badge tone="amber">Safety first</Badge>
          ...
          <Section id="principles" title="Recall principles (what actually makes it stick)">
            <ul className="list-disc space-y-2 pl-5" style={{ color: BRAND.ink }}>
              <li><strong>Predictability:</strong> your cue pays better than the environment—at first.</li>
              <li><strong>One clean cue:</strong> pick whistle or word and protect it from chatter.</li>
              <li><strong>Short & sweet:</strong> end with dopamine in the tank.</li>
              <li><strong>80% rule:</strong> only raise criteria when you can bet on success.</li>
              <li><strong>Distraction ladder:</strong> home → garden → quiet green → path → busy zones.</li>
            </ul>
            <ProTip title="Bank runs">10–20 easy recalls a day around the house are low-cost “deposits” that fund outdoor reliability later.</ProTip>
          </Section>
          ...
          <Section id="howto" title="How to teach recall (the clean sequence)">
            <ol className="list-decimal space-y-3 pl-6" style={{ color: BRAND.ink }}>
              <li><strong>Charge the marker:</strong> “Yes!” → treat (10 reps).</li>
              <li><strong>Pair the cue:</strong> cue, then 3–5 rapid treats at your knee.</li>
              <li><strong>Short distance:</strong> step back 1–2m, cue once, <em>move backwards</em>, feed at your knee.</li>
              <li><strong>Add delay:</strong> pause 1–2s after the cue before you move; reward at your knee.</li>
              <li><strong>Proofing:</strong> change one thing at a time (distance → mild distraction → combo).</li>
              <li><strong>Emergency cue:</strong> separate cue that always jackposts; once a week tops.</li>
            </ol>
          ...
          <Section id="drills" title="3-minute drills (set a timer, stack wins)">
            <p style={{ color: BRAND.slate }}>Micro-sessions keep things fun, prevent frustration and fit between life.</p>
          ...
          {/* Friendly CTA (no services/pricing claims) */}
          <BottomCta />
...
function ProgressBar({ progress }: { progress: number }) {
  return (
    <div className="h-[3px] w-full" style={{ background: BRAND.hair }}>
      <div className="h-full transition-[width] duration-150" style={{ width: `${Math.round(progress * 100)}%`, background: BRAND.brass }} />
    </div>
  );
}
...
function ArticleHero({ readingTime }: { readingTime: string }) {
  return (
    <header className="relative isolate overflow-hidden">
      <div className="absolute inset-5 -z-10 rounded-3xl bg-cover bg-center opacity-60" style={{ backgroundImage: "url('/saltaire-dog-river.png')" }} aria-hidden />
      ...
      <div className="h-10 w-full" style={{ background: 'radial-gradient(ellipse at top, rgba(200,155,60,0.18), transparent 60%)' }} aria-hidden />
    </header>
  );
}
...
function StickyToc({ toc, activeId, sticky = true }: { toc: TocItem[]; activeId: string; sticky?: boolean }) {
  ...
  return (
    <nav className={cx(sticky ? 'lg:sticky lg:top-20' : '', 'rounded-2xl border bg-white p-4 shadow-sm')} style={{ borderColor: BRAND.hair }} aria-label="On this page">
        ...
        {toc.map((item) => (
          <li key={item.id}>
            <button
              ...
              className={cx('group flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-sm transition-colors', activeId === item.id ? 'bg-[#EFEEE9]' : 'hover:bg-[#F3F2ED]')}
              style={{ color: BRAND.ink }}
              aria-current={activeId === item.id ? 'true' : undefined}
            >
              <span className="truncate">{item.label}</span>
              <span className="ml-3 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: activeId === item.id ? BRAND.brass : BRAND.hair }} />
            </button>
          </li>
        ))}
      </ul>
...
function TopMetaBar() {
  // Fixed date per request
  const date = '05 October 2025';
  return (
    ...
    <section id={id} className="scroll-mt-24">
      {title ? (
        <h2
          ...
          style={srOnlyTitle ? undefined : { color: BRAND.ink }}
        >
          {title}
        </h2>
      ) : null}
...
function Callout({ type, title, children }: { type: 'success' | 'warning' | 'info'; title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="not-prose mt-6 rounded-xl p-4 ring-1" style={{ background: palette.bg, boxShadow: '0 1px 0 rgba(255,255,255,.6) inset', color: BRAND.ink, borderColor: palette.ring }}>
...
function ProTip({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="not-prose mt-6 rounded-xl border bg-white p-4 shadow-sm" style={{ borderColor: BRAND.hair }}>
      <div className="flex items-start gap-3">
...
function Badge({ children, tone = 'emerald' }: { children: React.ReactNode; tone?: 'emerald' | 'amber' | 'sky' }) {
  ...
  const t = map[tone];
  return (
    <span className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold" style={{ background: t.bg, color: t.text, boxShadow: `inset 0 0 0 1px ${t.ring}` }}>
      {children}
    </span>
  );
}
...
function CalcRow({ label, description, value, onChange, min, max }: { label: string; description?: string; value: number; onChange: (v: number) => void; min: number; max: number }) {
  return (
    <div className="mt-3">
      <div className="flex items-center justify-between text-sm">
        <div>{label}</div>
  ...
  return (
    <div className="lift rounded-2xl border bg-white p-4" style={{ borderColor: BRAND.hair }}>
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full px-2 py-1 text-[11px] font-semibold" style={{ background: tone.chip, color: tone.text, boxShadow: `inset 0 0 0 1px ${tone.ring}` }}>
          {level}
        </span>
      </div>
      ...
      {items.map((it) => (
        <div key={it.step} className="rounded-2xl border bg-white p-4 shadow-sm" style={{ borderColor: BRAND.hair }}>
          <div className="flex items-center gap-3">
            <div className="inline-flex h-8 w-8 items-center justify-center rounded-full font-semibold" style={{ background: BRAND.stone, color: BRAND.ink }}>
              {it.step}
            </div>
...
function WeekPlanTabs() {
  ...
  return (
    <div className="not-prose mt-4">
      <div className="inline-flex rounded-xl border p-1" style={{ borderColor: BRAND.hair, background: '#F1EFE8' }}>
...
function TabBtn({ id, current, onClick, children }: { id: 'foundation' | 'stage1' | 'stage2'; current: string; onClick: (t: any) => void; children: React.ReactNode }) {
  ...
  return (
    <button
      onClick={() => onClick(id)}
      className={cx('rounded-lg px-3 py-1.5 text-sm font-semibold transition', active ? 'bg-white shadow-sm' : 'text-[#131415]/70 hover:text-[#131415]')}
      ...
      <div className="grid grid-cols-12 gap-1">
        {hours.map((h) => (
          <div key={h} className="space-y-2">
            <div className="h-10 rounded-md" style={{ background: `hsl(${hue(score(h))} 70% ${90 - score(h) * 18}%)` }} title={`${String(h).padStart(2, '0')}:00`} />
  ...
  const items = [
    {
      ...
      a: <>Lower distance to dogs until you’re back to 80% success. Use <em>sniff release</em>: come → treat → “go say hi” if safe.</>,
    },
    {
      q: 'Only comes with food visible',
      a: <>Switch to <strong>after-payment</strong>: cue → come → then hand dips to pouch. Mix in toys/play + environmental rewards.</>,
    },
    {
      q: 'Runs past you, doesn’t stop',
      a: <>Pay at your knee or behind your legs. Add a <em>target hand</em> near your thigh; feed at that spot so the dog docks fully.</>,
    },
    {
      q: 'Bolts to wildlife',
      a: <>That’s an emergency-cue moment. Train at longer distance first; if it happens, use your emergency cue once, jackpot, end session, rethink criteria.</>,
    },
  ];
  return (
      ...
      {items.map((it, i) => (
        <details key={i} className="group px-5 py-4">
          <summary className="flex cursor-pointer list-none items-start justify-between gap-6">
            ...
            <div className="mt-0.5 shrink-0 rounded-full border p-1 text-[#7B828A] transition group-open:rotate-180" style={{ borderColor: BRAND.hair }}>
  ...
  return (
    ...
    >
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        <div>
          <h3 className="text-2xl font-bold">Want a personalised recall plan?</h3>
          <p className="mt-1 max-w-xl text-white/85">
            Message us and we’ll share simple steps and local route ideas to suit your dog—no sales, just help.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link
...
function getArticleJsonLd(suggested: string) {
  const url = typeof window !== 'undefined' ? window.location.href : 'https://example.com/blog/perfect-recall-training-guide';
  const howTo = {
    '@type': 'HowTo',
